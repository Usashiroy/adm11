...
def demographicDistributionBarH(df, feat):
  ...
  group_dist.plot.barh(ax=ax, color='purple')
...
def demographicDistributionHist(df, feat):
  ...
  return px.histogram(df_subset, x=demog_cols[feat], color=demog_group_cols[feat], color_discrete_sequence=px.colors.sequential.PuRd)
