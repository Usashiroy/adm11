...
def loadAndCleanProfile(return_raw=False):
  ...
  profile_df["became_member_on"] = pd.to_datetime(profile_df["became_member_on"].astype(str)).astype('int64')
...
def createTranscriptFeatures(transcript_df, portfolio_df, profile_df):
  ...
  def calculate_atv(row):
      if row["cum_spending"] != 0 or row["transactions"] != 0:
          return row["cum_spending"] / row["transactions"]
      else:
          return 0.0

  transcript_feats["atv"] = transcript_feats.apply(calculate_atv, axis=1)
...
# Assuming transcript_feats is your DataFrame
  ...
  def calculate_offer_usage(row):
      if row["offers_received"] != 0:
          return row["offers_completed"] / row["offers_received"]
      else:
          return 0.0

  transcript_feats["offer_usage"] = transcript_feats.apply(calculate_offer_usage, axis=1)
...
def createTargets(transcript_feats, portfolio_df):
  ...
  # Calculate the future spending for each time window
  targ_df = target_df[['person', 'ts', 'amount']]
  for time_window in time_windows:
      # Rolling sums
      rs = targ_df.groupby("person", as_index=False).rolling(f"{time_window}h", on="ts", closed="left").sum()
  ...
  # Keep only offer received events and drop columns
...
def createDemographicGroups(profile):
  ...
  # Cohort groups
  demographics["became_member_on"] = pd.to_datetime(demographics["became_member_on"].astype(str)).astype('int64')
